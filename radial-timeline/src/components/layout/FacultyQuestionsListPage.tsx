...
import { MessageSquare, CheckCircle2, Clock, Search, ChevronRight, GraduationCap, Mail, Paperclip } from "lucide-react";
...
interface Question {
  ...
  studentId?: { _id: string; fullname: string; email: string; semester?: number };
  createdAt: string;
}

interface FacultyQListProps {
  filterStatus?: "Answered" | "Pending";
  pageTitle: string;
  pageDesc: string;
}

export function FacultyQuestionsListPage({ filterStatus, pageTitle, pageDesc }: FacultyQListProps) {
  ...
  const filtered = questions.filter((q) => {
    ...
    return (
      ...
      (q.studentId?.fullname || "").toLowerCase().includes(s)
    );
  });

  const icon = filterStatus === "Answered" ? CheckCircle2 : filterStatus === "Pending" ? Clock : MessageSquare;
  const color = filterStatus === "Answered" ? "teal" : filterStatus === "Pending" ? "amber" : "purple";
  const Icon = icon;
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        {questions.length > 0 && (
          <div className="relative mb-6">
            ...
            <input type="text" placeholder="Search by title, subject, or student name..."
              value={search} onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white text-sm placeholder:text-white/25 focus:outline-none focus:border-purple-500/50 transition-all" />
          </div>
        )}

        {!fetching && <p className="text-white/30 text-xs mb-4">{filtered.length} question{filtered.length !== 1 ? "s" : ""}{search ? " found" : ""}</p>}

        {fetching ? (
          <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="h-24 rounded-2xl bg-white/5 border border-white/5 animate-pulse" />)}</div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-20 rounded-2xl border border-white/5 bg-white/3">
            ...
            <p className="text-white/30 text-sm">{search ? "No questions match your search." : `No ${filterStatus?.toLowerCase() ?? ""} questions assigned to you yet.`}</p>
          </div>
        ) : (
              ...
              <div key={q._id} className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm overflow-hidden">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-1.5">
                        <span className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full border ${
                          q.status === "Answered" ? "text-teal-400 bg-teal-500/10 border-teal-500/30" : "text-amber-400 bg-amber-500/10 border-amber-500/30"
                        }`}>
                      ...
                      {q.studentId && (
                        <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                          <span className="flex items-center gap-1 text-white/40 text-xs">
                            <GraduationCap size={11} className="text-white/30" /> {q.studentId.fullname}
                          </span>
                          <a href={`mailto:${q.studentId.email}`} onClick={e => e.stopPropagation()}
                            className="flex items-center gap-1 text-white/30 text-xs hover:text-teal-400 transition-colors">
                            <Mail size={11} /> {q.studentId.email}
                          </a>
                          {q.studentId.semester && <span className="text-white/30 text-xs">Sem {q.studentId.semester}</span>}
                        </div>
                      )}
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button onClick={() => setExpanded(expanded === q._id ? null : q._id)}
                        ...
                        <ChevronRight size={15} className={`transition-transform ${expanded === q._id ? "rotate-90" : ""}`} />
                      </button>
                    </div>
                  </div>
                </div>
                ...
                {expanded === q._id && (
                  <div className="px-4 sm:px-5 pb-5 border-t border-white/10 pt-4 space-y-4">
                    <div>
                      <p className="text-white/50 text-xs uppercase tracking-wider mb-2 font-medium">Student Question</p>
                    ...
                    {q.answerText ? (
                      <div>
                        <p className="text-teal-400 text-xs uppercase tracking-wider mb-2 font-semibold">Your Answer</p>
                      ...
                      <button onClick={() => router.push(`/faculty/answer/${q._id}`)}
                        className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-purple-600 to-blue-600 text-white text-xs font-semibold hover:from-purple-500 hover:to-blue-500 transition-all shadow-md">
